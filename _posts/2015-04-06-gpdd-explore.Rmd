...
Unfortunately `dplyr` does not as yet directly support parallelization.  Most parallelization packages for R wrap around `apply` functions, and thus are not trivially adapted to the `dplyr` grammar.  If many cores are available it may still be faster to devolve the `group_by()` `data.frame` into a list of data frames and then apply in parallel; e.g.: 
...
fits <- mclapply(list_data, kalman))
