...
kalman <- function(df, ...){
  y <- df$logN
  ssg <- robust_fit("ssg", y, method='BFGS') 
  ssrw <- robust_fit("ssrw", y, method='BFGS') 
  g <- robust_fit("g", y, method='BFGS') 
  rw <- robust_fit("rw", y, method='BFGS')
...
fits <- reshape2::melt(fits_list, id=names(fits[[1]])) %>% rename(MainID = L1) %>% as_data_frame()
```
