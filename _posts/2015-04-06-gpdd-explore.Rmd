...
```{r}
fit_ssg <- function(y, 
                    init = c(dt = mean(y), Tt = 1, HHt = log(var(y)/2), GGt = log(var(y)/2)),
                      ...){
    o <- optim(init,
                 fn =  function(par, ...)
                   -fkf(dt = matrix(par[1]),
...
fit_ssrw <- function(y, init = c(dt=mean(y), HHt = log(var(y)/2), GGt = log(var(y)/2)), ...){
    o <- optim(init,
                 fn =  function(par, ...)
...
```{r}
robust_fit <- function(model = c("ssg", "ssrw", "g", "rw"), y, N = 50, ...){
  
  m <- switch(model,
              ssg = list(fit = fit_ssg, init =c(dt = mean(y), Tt = 1, HHt = log(var(y)/2), GGt = log(var(y)/2))),
              ssrw = list(fit = fit_ssrw, init =c(dt = mean(y), HHt = log(var(y)/2), GGt = log(var(y)/2))),
              g = list(fit = fit_g, init =c(dt = mean(y), Tt = 1, HHt = log(var(y)/2))),
              rw = list(fit = fit_rw, init =c(dt = mean(y), HHt = log(var(y)/2))))
  
  
  inits <- data.frame(id = 1:N, 
                      sapply(m$init, function(m) rnorm(N, m, sqrt(abs(m)))))  
  f <- function(init){
    o <- m$fit(y, init = init[-1], ...)
    data.frame(t(c(o$par, mloglik = o$value, converge = as.numeric(o$convergence), n=o$n, a0=o$a0)))
  }
  
  ## Apply the function to each initial condition, drop unconverged, and select only the best scoring run
  inits %>% group_by(id) %>% do(f(.)) %>% ungroup() %>% filter(converge == 0) %>% slice(which.min(mloglik)) 
}
```
...
Having defined a function that takes and returns a `data.frame`, `dplyr::do` gives us a consise syntax to apply this by group (recall `df` is already `group_by(MainID)`.) As the authors note, the robust fitting procedure is computationally intensive.
...
Note that BFGS is more problematic on some of the datasets: 
...
fits %>% 
  ...
  select(SSG, G) -> 
  c_values
ggplot(c_values, aes(SSG, G)) + geom_point() + 
  geom_abline(intercept=0, slope=1) + labs(title="c estimates")
```

Note the warning from dropping those points where the parameter value could not be estimated under one of the models.  Not surprisingly, it's almost always the full (SSG) model where convergence issues have arisen:


```{r}
fits %>% filter(converge != "0") %>% group_by(model) %>% summarize(count = n())
```
...
fits %>% 
  ...
  select(SSG, SSRW) -> 
  sigma2_values
ggplot(sigma2_values, aes(SSG, SSRW)) + 
  geom_point() + 
  geom_abline(intercept=0, slope=1) + 
  labs(title="sigma2 (observation error) estimates")
```
...
```{r}
multiple_fits <- function(model = c("ssg", "ssrw", "g", "rw"), y, N = 50, ...){
  ...
  ## Apply the function to each initial condition, drop unconverged, and select only the best scoring run
  inits %>% group_by(id) %>% do(f(.)) %>% ungroup() %>% select(-id) %>% filter(converge == 0)
}
...
```{r}
df %>% filter(MainID==5) %>% do(multiple_fits("ssg",  y=.$logN)) %>% select(dt, Tt, HHt, GGt, mloglik) %>% tidyr::gather(parameter, value, -MainID) -> all
...
```{r}
df %>% filter(MainID==1998) %>% do(multiple_fits("ssg",  y=.$logN)) %>% select(dt, Tt, HHt, GGt, mloglik) %>% tidyr::gather(parameter, value, -MainID) -> all
ggplot(all) + geom_histogram(aes(value)) + facet_wrap(~parameter, scales="free")
```


```{r}
multiple_fits("ssg",  y=Nile) %>% select(dt, Tt, HHt, GGt, mloglik) %>% tidyr::gather(parameter, value) -> all
ggplot(all) + geom_histogram(aes(value)) + facet_wrap(~parameter, scales="free")
```
