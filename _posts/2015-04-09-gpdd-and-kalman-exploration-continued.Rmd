...
```{r }
system.time(
  df %>% group_by(MainID) %>% do(bootstrap(., "ssrw", "ssg")) -> ssg_p_values
)
```

```{r }
system.time(
  df %>% group_by(MainID) %>% do(bootstrap(., "rw", "g")) -> g_p_values
  )
```
...
Or working around the current lack of parallelization in `dplyr::do()` for now:
...
```{r}
system.time(
  DD %>% group_by(MainID) %>% do_parallel(kalman) -> DD_fits
)
```

```{r}
system.time(
  DI %>% group_by(MainID) %>% do_parallel(kalman) -> DI_fits
)
```
...
## Rename factor levels and re-order
combined %>% transmutate(uncertainty = plyr::revalue(model, c(ssg = "accounting for uncertainty",
