...
### Configuring `knitr` with the `_build.R` script ###

- Caching:
- Figures:  I'm still not completely decided how to handle figures.  Embed into the html with data uris or link external files?  

## A docker image for the labnotebook ## 


The labnotebook images could be versioned with tags matching the year, such that `cboettig/labnotebook:2015` corresponded to a Dockerfile with software environment specific to building the repository. 

Running jekyll or servr from within docker requires changing the default host from `127.0.0.1` to `0.0.0.0`.
